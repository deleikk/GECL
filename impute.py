...
from utils import  preprocess, setup_seed
...
for sparity in ['0.95']:
    ...
    setup_seed(0)
    method = 'GECL'
    dir0 = '/home/kedelei/scBiG-main/scBiG-main/sim/'
    dir1 = '{}'.format(sparity)

    with h5py.File(os.path.join(dir0,'data_{}.h5'.format(sparity))) as data_mat:
    ...
    adata = preprocess(adata,args.hvg_top, hvg_top = False)
    ...
    ###training
    adata, record = run_GECL(adata, cl_type='cl_type', n_clusters=n_clusters, return_all=True,impute=True)
